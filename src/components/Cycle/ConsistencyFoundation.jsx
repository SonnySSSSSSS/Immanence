...
export function ConsistencyFoundation() {
    ...
    // Determine if Foundation Cycle is active (currentCycle exists)
    const isFoundationActive = !!currentCycle;

    return (
        <>
            {/* Active Cycle Summary - shown when cycle is active */}
            {currentCycle && (
                <motion.div
            ...
            {/* Programs Grid - ALWAYS VISIBLE */}
            <div className="grid grid-cols-2 gap-4 mb-4">
                {/* Card 1: Foundation Cycle */}
                <motion.div
                    className="p-6 rounded-3xl border transition-all duration-300 cursor-pointer group relative"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    onClick={() => !isFoundationActive && setShowCycleChoice(true)}
                    style={{
                        background: isLight
                            ? 'linear-gradient(135deg, rgba(255,255,255,0.7) 0%, rgba(255,255,255,0.4) 100%)'
                            : 'linear-gradient(135deg, #161625 0%, #1a1a2e 100%)',
                        borderColor: isFoundationActive 
                            ? (isLight ? 'rgba(180, 140, 90, 0.6)' : 'rgba(252, 211, 77, 0.6)')
                            : (isLight ? 'rgba(180, 140, 90, 0.25)' : 'rgba(255,255,255,0.1)'),
                        opacity: isFoundationActive ? 1 : 1,
                        cursor: isFoundationActive ? 'default' : 'pointer',
                    }}
                    onMouseEnter={(e) => {
                ...
                >
                    ...
                    {isFoundationActive && (
                        <div 
                            className="absolute top-3 right-3 px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-wider"
                            style={{
                    ...
                    <div className="flex flex-col h-full">
                        ...
                        <div className="w-full py-2 rounded-xl text-[10px] font-bold text-center uppercase tracking-widest transition-colors"
                            style={{
                                background: isFoundationActive 
                                    ? (isLight ? 'rgba(100, 150, 100, 0.15)' : 'rgba(100, 150, 100, 0.2)')
                                    : (isLight ? 'rgba(180, 140, 90, 0.1)' : 'rgba(252, 211, 77, 0.1)'),
                                color: isFoundationActive
                                    ? (isLight ? 'rgba(60, 120, 60, 0.9)' : '#86efac')
                                    : (isLight ? 'rgba(140, 100, 40, 0.8)' : '#fcd34d')
                            }}
                        >
                            {isFoundationActive ? 'In Progress' : 'Begin'}
                        </div>
                    </div>
                </motion.div>
