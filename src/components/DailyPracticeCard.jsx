...
export function DailyPracticeCard({ onStartPractice, onViewCurriculum, onNavigate, hasPersistedCurriculumData, onStartSetup, onboardingComplete: onboardingCompleteProp, practiceTimeSlots: practiceTimeSlotsProp, isTutorialTarget = false, showPerLegCompletion = true, showDailyCompletionNotice = false, showSessionMeter = true, debugShadowOff = false, debugBlurOff = false, debugBorderOff = false, debugMaskOff = false, devCardActive = null, devCardCarouselId = null }) {
    ...
    useEffect(() => {
        ...
        const handleClickOutside = (e) => {
            if (
                pathActionsMenuRef.current?.contains(e.target) ||
                pathActionsPopupRef.current?.contains(e.target)
            ) {
                return;
            }
            setIsPathActionsMenuOpen(false);
        };

        document.addEventListener('mousedown', handleClickOutside);
        return () => document.removeEventListener('mousedown', handleClickOutside);
    }, [isPathActionsMenuOpen]);
                                ...
                                {!activePathObj ? (
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center justify-between" style={{ position: 'relative', zIndex: 20 }}>
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => setIsPathActionsMenuOpen(!isPathActionsMenuOpen)}
                                                ref={pathActionsMenuRef}
                                                data-testid="path-actions-trigger"
                                            ...
                                            {isPathActionsMenuOpen && (
                                                <div
                                                    ref={pathActionsPopupRef}
                                                    style={{
                                                        position: 'absolute',
                                                        top: '100%',
                                                        right: 0,
                                                        marginTop: '6px',
                                                        background: isLight ? 'rgba(236, 246, 248, 0.95)' : 'rgba(20, 25, 35, 0.96)',
                                                        border: `1px solid ${isLight ? 'rgba(139, 115, 85, 0.25)' : 'var(--accent-20)'}`,
                                                        borderRadius: '8px',
                                                        boxShadow: isLight
                                                ...
                                                >
                                                    <button
                                                        ...
                                                        onMouseEnter={e => e.currentTarget.style.background = isLight ? 'rgba(139, 159, 136, 0.10)' : 'rgba(120, 255, 180, 0.08)'}
