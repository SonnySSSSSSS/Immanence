...
const mockVideos = [
  { id: "ancestors-ddos", title: "Ancestors as DDoS Protection", length: "12:34" },
  { id: "xom-reality-sync", title: "Xóm Culture as Reality Sync", length: "09:18" },
];
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Title section */}
        <div className="px-8 py-6 border-b border-[var(--accent-10)] text-center">
          <h2
            className="text-2xl font-semibold mb-2"
            style={{ fontFamily: 'Cinzel, Georgia, serif', color: 'var(--accent-color)', letterSpacing: '0.03em' }}
          >
            {sanitizedTitle}
          </h2>
          {sanitizedSubtitle && (
        ...
        <div
          ref={contentRef}
          className="flex-1 overflow-y-auto px-10 py-8 prose-content"
          style={{ fontFamily: 'Crimson Pro, Georgia, serif', fontSize: '17px', lineHeight: '1.8', color: 'rgba(253,251,245,0.92)' }}
        >
          {sanitizedExcerpt && (
            <blockquote
...
function CategoryCard({ category, isSelected, onClick }) {
  return (
    <button
      ...
      style={{
        background: isSelected ? 'var(--accent-10)' : 'rgba(0,0,0,0.2)',
        border: isSelected ? '1px solid var(--accent-40)' : '1px solid var(--accent-15)',
        boxShadow: isSelected ? '0 0 30px var(--accent-10)' : 'none',
        minHeight: '140px'
      }}
    >
      <div className="flex items-start gap-2 mb-3">
        <span className="flex-shrink-0" style={{ color: 'var(--accent-color)', fontSize: '12px' }}>◎</span>
        <span
...
export function WisdomSection() {
  ...
  const toggleBookmark = (chapterId) => {
    setBookmarkedIds(prev => prev.includes(chapterId) ? prev.filter(id => id !== chapterId) : [...prev, chapterId]);
  };
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1 relative">
              <input
                ...
                className="w-full bg-black/30 border border-[var(--accent-15)] rounded-xl px-4 py-3 text-[13px] placeholder:text-white/40 focus:outline-none focus:border-[var(--accent-40)]"
  ...
  const renderVideosView = () => (
    <div className="space-y-3">
      {mockVideos.map((video) => (
        <div key={video.id} className="px-5 py-4 rounded-2xl border transition-all" style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid var(--accent-15)' }}>
          <div className="flex items-center justify-between">
            <div>
              <div className="text-[13px] font-medium" style={{ color: 'rgba(253,251,245,0.85)' }}>{video.title}</div>
              <div className="text-[11px] mt-1" style={{ color: 'rgba(253,251,245,0.5)' }}>{video.length}</div>
            </div>
            <button className="px-4 py-2 rounded-full text-[11px] border transition-all" style={{ border: '1px solid var(--accent-20)', color: 'rgba(253,251,245,0.7)' }}>
              Play
            </button>
          </div>
        </div>
  ...
  return (
    <>
      ...
      <div className="w-full max-w-5xl mx-auto">
        <div className="rounded-3xl border border-[var(--accent-15)] bg-black/40 backdrop-blur-xl px-5 py-5 space-y-5 shadow-[0_0_60px_rgba(0,0,0,0.5)]">
          {/* Tabs */}
