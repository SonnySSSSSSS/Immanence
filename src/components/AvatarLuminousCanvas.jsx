...
function drawNebulaWisps(ctx, time) {
  // REDUCED OPACITY: 50% reduction to reduce shine
  const wisps = [
    { angle: time * 0.0001, dist: 120, size: 150, opacity: 0.02 },
    { angle: time * 0.00015 + 2, dist: 150, size: 180, opacity: 0.015 },
    { angle: time * 0.00008 + 4, dist: 100, size: 120, opacity: 0.025 },
  ];
...
function drawSacredGeometry(ctx, radius) {
  // REDUCED OPACITY: from 0.10 to 0.05
...
function drawUnifyingGlow(ctx) {
  // REDUCED OPACITY: ~60% reduction to reduce shine obscuring avatar
  const gradient = ctx.createRadialGradient(
    0, 0, 100,   // Start inside rune ring
    0, 0, 320    // Extend past outer decorative ring
  );

  gradient.addColorStop(0, 'rgba(253, 224, 71, 0.06)');
  gradient.addColorStop(0.3, 'rgba(253, 224, 71, 0.03)');
  gradient.addColorStop(0.6, 'rgba(200, 160, 60, 0.015)');
  gradient.addColorStop(1, 'transparent');
...
function drawAtmosphericHaze(ctx, time) {
  // REDUCED OPACITY: from 0.06 to 0.025
  const hazePoints = [
  ...
  hazePoints.forEach(h => {
    ...
    gradient.addColorStop(0, 'rgba(180, 140, 60, 0.025)');
    gradient.addColorStop(1, 'transparent');
