...
const RitualStepDisplay = ({ step, stepIndex, totalSteps, isPaused }) => {
    ...
    return (
        <div className="w-full h-full flex flex-col md:flex-row gap-4 p-4 relative overflow-visible">
            ...
            <div className="flex-1 flex flex-col gap-4 bg-black/60 backdrop-blur-md p-6 rounded-2xl border border-white/10 shadow-xl md:min-w-[400px] max-h-[60vh] md:max-h-full overflow-y-auto">
                {/* Header */}
                <div className="flex justify-between items-baseline border-b border-white/10 pb-3">
                    <h3 className="text-2xl font-light text-[var(--accent-primary)] font-h1">
                        {step.name}
                    </h3>
                    <span className="text-sm text-[var(--accent-muted)] font-mono">
