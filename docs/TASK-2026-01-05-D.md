...
## Menu Definition (Strict Scope Boundary)

**A "menu surface" is defined as:**
1. **Grid of selectable cards/buttons** where each item represents a navigation choice, practice option, or mode selection
2. **Action sheets or option lists** where user picks one item from multiple choices
3. **Bottom sheets or drawer menus** with multiple tappable options
...
document.querySelectorAll('h3, h4, .menu-title, [class*="title"]').forEach(el => {
  if (el.scrollWidth > el.clientWidth) {
...
| Route/Screen | Component + File Path | Failure Mode | Suspected Cause | Proposed Guardrail Fix |
|--------------|----------------------|--------------|-----------------|------------------------|
| Home Hub | `HomeHub.jsx` - Card swiper | Horizontal overflow | Fixed 430px card width | Reduce to max(90vw, 300px) at ≤360px |
| Practice Section | `PracticeConfigCard.jsx` - Practice type grid | Text truncation | `whitespace-nowrap` on titles | Remove nowrap, add `line-clamp-2` |
| Navigation | `PathSelectionGrid.jsx` - 3-column grid | Cards clipped | `grid-cols-3` at small widths | Change to `grid-cols-2` at ≤360px |
| Application | `FourModesHome.jsx` - 2x2 mode grid | Button text cut | Fixed height + single-line assumption | Use `min-height`, allow wrap |
| Wisdom | `CodexChamber.jsx` - Treatise list | Bottom items hidden | Fixed height without scroll | Add `overflow-y-auto` or reduce height |
| ... | ... | ... | ... | ... |
...
### Rule 6: Viewport-Relative Widths
**Default:** Card widths use `min(Xpx, 90vw)` not fixed px
...
// Modal
<div className="max-w-[90vw] sm:max-w-md md:max-w-lg">

// Inline style
<div style={{
  width: '100%',
  maxWidth: window.innerWidth <= 360 ? '90vw' : '430px'
}}>
```
...
const checkOverflow = () => {
  ...
  document.querySelectorAll('h1, h2, h3, h4, button, a').forEach(el => {
    if (el.scrollWidth > el.clientWidth + 1 && !el.classList.contains('truncate')) {
      failures.push({
...
<div style={{ maxWidth: '430px', margin: '0 auto' }}>
  <div className="flex gap-4">
    {cards.map(card => (
      <div key={card.id} className="h-40 flex-shrink-0" style={{ width: '430px' }}>
        {/* card content */}
      </div>
    ))}
  </div>
