...
- Cancel → return to Programs grid with NO state changes
- Completion → generate payload and pass to curriculum handler
...
- **Actions:** Continue → Step 2, Cancel → exit
...
- **Actions:** Continue → Step 3, Back → Step 1, Cancel → exit
...
- **Actions:** Continue → Step 4, Back → Step 2, Cancel → exit
...
- **Actions:** Start Program → Complete, Back → Step 3, Cancel → exit
...
**IMPORTANT:** Do NOT modify `curriculumStore.js`. Use the existing API as-is.

**Current Signature:**
```javascript
completeOnboarding: (timeSlots = [], thoughts = []) => {
    // Two separate array parameters (not an object)
...
const handleComplete = () => {
    // Call with TWO separate array arguments (match existing signature exactly)
    completeOnboarding(selectedTimeSlots, thoughtEntries);
    onClose();
};
```

**Note:** The signature takes two separate arrays, NOT a single object. Pass `timeSlots` as first argument, `thoughts` as second argument.
...
- `src/state/curriculumStore.js` - **NO EDITS** (use existing API only, do not modify the store)
...
- ANY other state stores not explicitly listed in ALLOWLIST
...
3. Click Back → Verify Step 1 returns
4. Click Continue → Step 2 again
...
- Complete → curriculumStore.completeOnboarding()
...
1. **Icon Choice:** Prefer 🌊 (wave), 💭 (thought bubble), 🧘 (meditation), or 🪷 (lotus)?
